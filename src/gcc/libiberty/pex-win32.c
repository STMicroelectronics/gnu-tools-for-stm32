  ...
  if (wcslen (cmdline) > 32767)
    {
      ...
      wchar_t *wresponse_file = handle_long_path_utf8 (response_file);
      if (!wresponse_file)
        goto exit;

      response_file = wchar_to_utf8 (wresponse_file);
      free (wresponse_file);
      if (!response_file)
        goto exit;
      ...
      wchar_t *response_argv[3] = {norm_argv[0], wresponse_arg, NULL};
      free (cmdline);
